    ...
    <div id="content">
      ...
      <div class="container-fluid">
        <div class="card shadow mb-4">
          ...
          <div class="card-body">
            <form action="<?=base_url('UpdateInventoryProduct/'. $products->inventory_id) ?>" method="post">
              <!--FOR SERVICE NAME-->
              <div class="form-group mt-2">
                <label for="">Product Name</label>
                <select class="form-control" name="service_name">
                  <?php if ($products) : ?>
                    <option value="<?= $products->inventory_name ?>">
                      <?= $products->inventory_name ?>
                    </option>
                  <?php endif; ?>
                  <option class="font-weight-bold text-dark" value="">Select Service</option>
                  <optgroup label="CASKET:">
                      <?php foreach ($caskets as $casket) : ?>
                          <option value="<?= $casket->casket_name ?>">
                              <?= $casket->casket_name ?>
                          </option>
                      <?php endforeach; ?>
                  </optgroup>
                  <!-- Other Services -->
                  <optgroup label="URN:">
                      <?php foreach ($urns as $urn) : ?>
                          <option value="<?= $urn->urn_name ?>">
                              <?= $urn->urn_name ?>
                          </option>
                      <?php endforeach; ?>
                  </optgroup>
                  <!-- Additional Product -->
                  <optgroup label="ASSOCIATED PRODUCT/ADDITIONAL PRODUCT:">
                      <?php foreach ($additional_prods as $additional_prod) : ?>
                          <option value="<?= $additional_prod->associated_product_name ?>">
                              <?= $additional_prod->associated_product_name ?>
                          </option>
                      <?php endforeach; ?>
                  </optgroup>
                </select>
              ...
              <div class="form-group mt-2">
                ...
                <input type="text" name="unit_price" value="<?= $products->unit_price ?>" class="form-control">
                <small class="text-danger"><?=form_error('unit_price'); ?></small>
              </div>
              ...
              <div class="form-group mt-2">
                  ...
                  <select class="form-control" name="inventory_category">
                      <option value=""></option>
                      <option value="Urn" <?= ($products->inventory_category == 'Urn') ? 'selected' : '' ?>>Urn</option>
                      <option value="Casket" <?= ($products->inventory_category == 'Casket') ? 'selected' : '' ?>>Casket</option>
                      <option value="Additional Product" <?= ($products->inventory_category == 'Additional Product') ? 'selected' : '' ?>>Additional Product</option>
                  </select>
                  <small class="text-danger"><?= form_error('inventory_category') ?></small>
              </div>

              <div class="form-group">
                <label for="">Quantity in Stock</label>
                <input type="number" name="inventory_stock" class="form-control" value="<?php echo $products->inventory_stock; ?>">
                <small class="text-danger"><?= form_error('inventory_stock')?></small>
              </div>

              <div class="form-group">
                <label for="">Date Restock</label>
                <input class="form-control" type="date" name="date_restock" value="<?php echo $products->date_restock; ?>">
                <small class="text-danger"><?= form_error('date_restock')?></small>
              </div>
